...
clin_mat<- matrix(nrow=13, ncol=5)
eff_mat<- matrix(nrow=13, ncol=5)
wk_mat<- matrix(nrow=13, ncol=9)
expect_zeta<- matrix(nrow=13, ncol=9)
marginale_mat<- matrix(nrow=13, ncol=9)
...
set.seed(20)
  ...
  while(((i-1)*cohortsize+1) <= ncohort)
  {
    ...
    marginale_mat[i,]<- marginale
    expect_zeta[i,]<- sapply(1:9, function (k) integrate(bcrmht,lower=-Inf,upper=Inf, skeletone[k,], ye, n, se,abs.tol = 0)$value/marginale[k])
...
# plot(1:6, clin_mat[1,], ylim=c(0,1), type="l")
# for(i in 2:12){
#   lines(1:6, clin_mat[i,])
# }
# 
# plot(1:6, eff_mat[1,], ylim=c(0,1), type="l")
# for(i in 2:12){
#   lines(1:6, eff_mat[i,])
# }


data<-tidyr::pivot_longer(data.frame(wk_mat[c(1,3,6,9,13),]), cols = everything(), names_to = "Column", values_to = "Value")
...
pdf("N134wk.pdf", height=4, width=7)
ggplot(data, aes(x=Skeleton, y=Value, colour=Cohort))+geom_line(size=1.5)+ 
  theme_classic(base_size = 25)+theme(panel.grid.major = element_blank(), panel.grid.minor=element_blank())+ labs(x = "Skeleton", y =expression(w(k ~ "|" ~ D[n])))+
...
k<-1
mat<-sapply(1:9, function (j) skeletone[j,]^exp(expect_zeta[k,j]))
...
pdf("N134z.pdf", height=4.5, width=7)
